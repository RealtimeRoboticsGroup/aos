...
```
bazel build -c opt //documentation/examples/ping_pong:pong //documentation/examples/ping_pong:ping //aos:aos_dump
```
...
3. In a third window, explore the message stream using `aos_dump`. Some things you can do:
   1. List the channels: `bazel-bin/aos/aos_dump --config bazel-bin/documentation/examples/ping_pong/pingpong_config.json`
   2. Listen to a specific topic on a channel-- copy one of the channels listed in the first step and put it at the end of the aos_dump command (e.g., "/test aos.examples.Ping") `bazel-bin/aos/aos_dump --config bazel-bin/documentation/examples/ping_pong/pingpong_config.json /test aos.examples.Ping`
   3. Listen to multiple topics on a channel (e.g., all the topics published on "/test") `bazel-bin/aos/aos_dump --config bazel-bin/documentation/examples/ping_pong/pingpong_config.json /test`
...
```
alias aos_dump='bazel-bin/aos/aos_dump --config bazel-bin/documentation/examples/ping_pong/pingpong_config.json'
```
...
```
bazel-bin/aos/events/logging/logger_main --config bazel-bin/documentation/examples/ping_pong/pingpong_config.json --logging_folder /tmp/log_folder/
```
