...
Completion callbacks routinely reenter the ring: a timer callback reschedules, a cancel drains. (Not `Poll()` itself, which is fatal to reenter from anywhere inside a `Poll()` — see `in_poll_`.) Dispatching straight off the CQ iteration would let that reentry advance the CQ head under the live scan, or recurse into nested dispatch. So handling is two-phase:
