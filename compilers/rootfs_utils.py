...
def scoped_tmpdir_tarball(image):
    ...
    """
    with tempfile.TemporaryDirectory() as tempdir:
        print(f'Extracting original image to {tempdir}')
        subprocess.run([
            "tar",
            "xf",
            image,
...
def read_linker_script(filename):
    ...
    try:
        ...
        print(f"Error: Linker script file not found: {filename}")
        return None
...
class Filesystem:
    ...
    def object_dependencies(self, obj: str) -> str:
        ...
        if not is_elf_file(filename):
            ...
            return parse_linker_script(read_linker_script(filename)), []
...
def generate_build_file(filesystem, packages_to_eval, template_filename):
    ...
    while objs_to_eval:
        ...
        deps, runpaths = filesystem.object_dependencies(obj)
        resolved_deps = []
        for d in deps:
        ...
        rule_deps = ''.join([
            ...
            for d in resolved_deps if not d.endswith('/libc.so.6')
        ])
        rules.append(
            f'cc_library(\n    name = "{rule_name}-lib",\n    srcs = ["{obj[1:]}"],\n    deps = [\n{rule_deps}    ],\n)'
        )
    ...
    for pkg in filesystem.pkgcfg:
        try:
            ...
            print('Failed to instantiate package',
                  repr(pkg),
                  e,
                  file=sys.stderr)
            pass
