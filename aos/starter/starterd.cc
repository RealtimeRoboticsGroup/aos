  ...
  if (!absl::GetFlag(FLAGS_user).empty()) {
    ...
    std::vector<__gid_t> groups;
    {
      ...
      if (user_data != nullptr) {
        ...
        getgrouplist(user_data->pw_name, gid, NULL, &ngroups);
        groups.resize(ngroups);
        getgrouplist(user_data->pw_name, gid, groups.data(), &ngroups);
        for (int i = 0; i < ngroups; i++) {
          struct group *gr = getgrgid(groups[i]);
          PCHECK(gr != nullptr);

          LOG(INFO) << "  Adding supplimental group of " << gr->gr_name;
        }
      } else {
    ...
    // Change the supplimental groups of the user we're running as.
    PCHECK(setgroups(groups.size(), groups.data()) == 0) << ": Failed to set groups";
