...
def sysroot_paths_dict(rctx, sysroot_dict, use_absolute_paths):
    ...
    for (target_os, target_arch) in _supported_targets:
        ...
        # Normalize the sysroot path to have no trailing slash so that
        # downstream consumers can safely write `sysroot_path + "/sub/dir"`
        # regardless of whether the sysroot label points at the root
        # package of an external repo (which yields a trailing-slashed
        # `workspace_root + "/" + ""`) or a sub-package (which does not).
        # Preserve a bare "/" since that means "use the host filesystem
        # root as the sysroot".
