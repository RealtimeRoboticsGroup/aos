...
bool PoseIsValid(const TargetMapper::TargetPose &target_pose, double pose_error,
  ...
  if (static_cast<TargetMapper::TargetId>(target_pose.id) <
      ...
      static_cast<TargetMapper::TargetId>(target_pose.id) >
          absl::GetFlag(FLAGS_max_target_id) || target_pose.id == 6) {
...
void ExtrinsicsMain(const NodeList &node_list,
  ...
  std::vector<std::pair<TimestampedCameraDetection, TimestampedCameraDetection>>
      detection_list;
  std::vector<TimestampedCameraDetection> two_board_extrinsics_list_with_outliers;
  std::vector<TimestampedCameraDetection> two_board_extrinsics_list;
  ...
  for (const CameraNode &camera_node : node_list.cameras) {
    ...
    event_loop->MakeWatcher(
        ...
        [&reader, event_loop, camera_node, &last_observation, &detection_list,
         &two_board_extrinsics_list_with_outliers, &vis_robot, &camera_colors, &ordering_map,
  ...
  VLOG(1) << "Base id for two_board_extrinsics_list_with_outliers is " << base_target_id;
