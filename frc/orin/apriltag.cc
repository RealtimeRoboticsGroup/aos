...
size_t InputFormatToChannels(const ImageFormat image_format) {
  if (image_format == ImageFormat::MONO8) {
    return 1;
  }
  if (image_format == ImageFormat::MONO16) {
    return 2;
  }
  if (image_format == ImageFormat::YUYV422) {
    return 2;
  }
  if (image_format == ImageFormat::BGR8) {
    return 3;
  }
  if (image_format == ImageFormat::BGRA8) {
    return 4;
  }
...
GpuDetector::GpuDetector(size_t width, size_t height,
    ...
    : width_(width),
      ...
      input_size_(width * height * InputFormatToChannels(image_format)),
      ...
      color_image_device_(
          width * height_ *
          InputFormatToChannels(
              image_format)),  // TODO : try allocating as write-combined?
  ...
  if (image_format == ImageFormat::MONO8) {
    threshold_ = std::make_unique<Threshold<ImageFormat::MONO8>>();
  } else if (image_format == ImageFormat::MONO16) {
    threshold_ = std::make_unique<Threshold<ImageFormat::MONO16>>();
  } else if (image_format == ImageFormat::YUYV422) {
    threshold_ = std::make_unique<Threshold<ImageFormat::YUYV422>>();
  } else if (image_format == ImageFormat::BGR8) {
    threshold_ = std::make_unique<Threshold<ImageFormat::BGR8>>();
  } else if (image_format == ImageFormat::BGRA8) {
    threshold_ = std::make_unique<Threshold<ImageFormat::BGRA8>>();
  } else {
    // TODO : Probably need a throw or assert here
  }
  ...
  // TODO - this supposedly speeds up kernel submits, but needs to
  // benchmarking to see if it matters (or is worth it vs. the CPU
  // load hit)
...
class AddThetaToIndexPoint {
 public:
  ...
  __host__ __device__ __forceinline__ IndexPoint operator()(IndexPoint a) {
    ...
    float theta = (atan2f(a.y() - extents.cy(), a.x() - extents.cx()) +
...
class SelectBlobs {
 public:
  ...
  __host__ __device__ __forceinline__ bool operator()(
    ...
    if (static_cast<uint32_t>(extents.max_x - extents.min_x) *
            static_cast<uint32_t>(extents.max_y - extents.min_y) <
        tag_width_) {
      return false;
    }
...
void GpuDetector::Detect(const uint8_t *image) {
  ...
  event_timings_.start("union_markers_size_devize_memset",
                       memset_stream_.get());
...

  {
    ...
    event_timings_.start("num_compressed_union_marker_pair_memcpy_d2h",
                         stream_.get());
  ...
  {
    if (image_format_ == ImageFormat::MONO8) {
      ...
      //        Eventually make gray_image_host_ptr point to an unpinned mem
      //        buffer?
  ...
  {
    ScopedEventTiming t(event_timings_, "InclusiveScanByKeyMaskBlobSizes",
                        stream_.get());
  ...
  {
    ScopedEventTiming t(event_timings_, "cub::DeviceRadixSort::SortKeysBlobs",
                        stream_.get());
  ...
  {
    ScopedEventTiming t(event_timings_, "cub::InclusiveScanMaskBlob",
                        stream_.get());
    // Now that we have the dot products, we need to rewrite the extents for the
    // post-thresholded world so we can find the start and end address of blobs
    // for fitting lines.
...

  {
    ...
    CHECK_CUDA(cub::DeviceRadixSort::SortKeys(
        ...
        *num_compressed_peaks_host_.get(), decomposer, 0,
        PeakDecomposer::kBitsInKey, stream_.get()));
...

  {
    ...
    apriltag::FitQuads(
        sorted_compressed_peaks_device_.get(),
        *num_compressed_peaks_host_.get(), peak_extents_device_.get(),
        *num_quad_peaked_quads_host_.get(), line_fit_points_device_.get(),
        tag_detector_->qtp.max_nmaxima, selected_extents_device_.get(),
        tag_detector_->qtp.max_line_fit_mse,
...

  {
    ...
    fit_quads_device_.MemcpyAsyncTo(
        fit_quads_host_.data(), *num_quad_peaked_quads_host_.get(), &stream_);
  ...
  VLOG(1) << "Found " << num_compressed_union_marker_pair_host_.get()[0]
          << " items";
  VLOG(1) << "Selected " << num_selected_blobs_host_.get()[0]
          << " right side out points";
  VLOG(1) << "Found compressed runs: " << num_quads_host_.get()[0];
  VLOG(1) << "Peaks " << num_compressed_peaks_host_.get()[0] << " peaks";
  VLOG(1) << "Peak Selected blobs " << num_quad_peaked_quads_host_.get()[0]
          << " quads";
