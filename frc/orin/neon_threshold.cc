...
void NeonThreshold::ThresholdAndDecimate(
  ...
  // Unfortunately, the nieve algorithm is slow.  We can do better with some
  ...
  for (size_t h = 0; h < height_ / 8; h += 1) {
    ...
    for (; w < width_ / 8; w += 2) {
      ...
      // min/max.  We'd need to process 8 blocks at a time to do that efficiently.
      const uint8_t max0 =
      ...
      // Min/max need to be delayed by 1 iteration so we can write the filtered ones out.
      //
