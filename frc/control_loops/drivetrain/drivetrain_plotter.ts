...
export function plotDrivetrain(conn: Connection, element: Element): void {
  ...
  const teleopPlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  const modePlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  positionPlot.plot
    .getAxisLabels()
    .setTitle('Estimated Relative Position ' + 'of the Drivetrain');
  positionPlot.plot.getAxisLabels().setXLabel(TIME);
  positionPlot.plot.getAxisLabels().setYLabel('Relative Position (m)');
  const leftPosition = positionPlot.addMessageLine(status, [
    'estimated_left_position',
  ]);
  leftPosition.setColor(RED);
  const rightPosition = positionPlot.addMessageLine(status, [
    'estimated_right_position',
  ]);
  rightPosition.setColor(GREEN);
  const leftPositionGoal = positionPlot.addMessageLine(status, [
    'profiled_left_position_goal',
  ]);
  leftPositionGoal.setColor(BLUE);
  const rightPositionGoal = positionPlot.addMessageLine(status, [
    'profiled_right_position_goal',
  ]);
  ...
  const leftVoltageError = voltageErrors.addMessageLine(status, [
    'left_voltage_error',
  ]);
  leftVoltageError.setColor(RED);
  const rightVoltageError = voltageErrors.addMessageLine(status, [
    'right_voltage_error',
  ]);
  rightVoltageError.setColor(GREEN);

  const ekfLeftVoltageError = voltageErrors.addMessageLine(status, [
    'localizer',
    'left_voltage_error',
  ]);
  ekfLeftVoltageError.setColor(PINK);
  const ekfRightVoltageError = voltageErrors.addMessageLine(status, [
    'localizer',
    'right_voltage_error',
  ]);
  ...
  const ffLeftVoltage = otherVoltages.addMessageLine(status, [
    'poly_drive_logging',
    'ff_left_voltage',
  ]);
  ffLeftVoltage.setColor(RED);
  ffLeftVoltage.setPointSize(0);
  const ffRightVoltage = otherVoltages.addMessageLine(status, [
    'poly_drive_logging',
    'ff_right_voltage',
  ]);
  ...
  const uncappedLeftVoltage = otherVoltages.addMessageLine(status, [
    'uncapped_left_voltage',
  ]);
  uncappedLeftVoltage.setColor(RED);
  uncappedLeftVoltage.setDrawLine(false);
  const uncappedRightVoltage = otherVoltages.addMessageLine(status, [
    'uncapped_right_voltage',
  ]);
  ...
  const ssLeftVelocityGoal = velocityPlot.addMessageLine(status, [
    'profiled_left_velocity_goal',
  ]);
  ssLeftVelocityGoal.setColor(PINK);
  ssLeftVelocityGoal.setPointSize(0.0);
  const ssRightVelocityGoal = velocityPlot.addMessageLine(status, [
    'profiled_right_velocity_goal',
  ]);
  ssRightVelocityGoal.setColor(CYAN);
  ssRightVelocityGoal.setPointSize(0.0);

  const polyLeftVelocity = velocityPlot.addMessageLine(status, [
    'poly_drive_logging',
    'goal_left_velocity',
  ]);
  ...
  const polyRightVelocity = velocityPlot.addMessageLine(status, [
    'poly_drive_logging',
    'goal_right_velocity',
  ]);
  ...
  velocityPlot2.plot
    .getAxisLabels()
    .setTitle('Trajectory and Localizer Velocity Plots');
  velocityPlot2.plot.getAxisLabels().setXLabel(TIME);
  velocityPlot2.plot.getAxisLabels().setYLabel('Wheel Velocity (m/s)');

  const splineLeftVelocity = velocityPlot2.addMessageLine(status, [
    'trajectory_logging',
    'left_velocity',
  ]);
  ...
  const splineRightVelocity = velocityPlot2.addMessageLine(status, [
    'trajectory_logging',
    'right_velocity',
  ]);
  ...
  const ekfLeftVelocity = velocityPlot2.addMessageLine(status, [
    'localizer',
    'left_velocity',
  ]);
  ekfLeftVelocity.setColor(RED);
  ekfLeftVelocity.setPointSize(0.0);
  const ekfRightVelocity = velocityPlot2.addMessageLine(status, [
    'localizer',
    'right_velocity',
  ]);
  ...
  const splineVelocityOffset = velocityPlot2.addMessageLine(status, [
    'localizer',
    'longitudinal_velocity_offset',
  ]);
  splineVelocityOffset.setColor(BROWN);
  splineVelocityOffset.setPointSize(0.0);

  const splineLateralVelocity = velocityPlot2.addMessageLine(status, [
    'localizer',
    'lateral_velocity',
  ]);
  ...
  const splineYaw = yawPlot.addMessageLine(status, [
    'trajectory_logging',
    'theta',
  ]);
  ...
  const downEstimatorYaw = yawPlot.addMessageLine(status, [
    'down_estimator',
    'yaw',
  ]);
  ...
  const expectedAccelX = accelPlot.addMessageLine(status, [
    'down_estimator',
    'expected_accel_x',
  ]);
  expectedAccelX.setColor(RED);
  expectedAccelX.setPointSize(0);
  const expectedAccelY = accelPlot.addMessageLine(status, [
    'down_estimator',
    'expected_accel_y',
  ]);
  expectedAccelY.setColor(GREEN);
  expectedAccelY.setPointSize(0);
  const expectedAccelZ = accelPlot.addMessageLine(status, [
    'down_estimator',
    'expected_accel_z',
  ]);
  ...
  const gravity_magnitude = accelPlot.addMessageLine(status, [
    'down_estimator',
    'gravity_magnitude',
  ]);
  ...
  xPositionPlot
    .addMessageLine(status, ['trajectory_logging', 'x'])
    .setColor(GREEN);
  ...
  yPositionPlot
    .addMessageLine(status, ['trajectory_logging', 'y'])
    .setColor(GREEN);
  ...
  // IMU States
  const imuStatePlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  const faultedLine = imuStatePlot.addMessageLine(status, [
    'zeroing',
    'faulted',
  ]);
