...
export function plotRobotState(conn: Connection, element: Element): void {
  ...
  const robotStatePlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  robotStatePlot
    .addMessageLine(robotState, ['outputs_enabled'])
    .setColor(CYAN)
    .setDrawLine(false);
  ...
  // Battery Voltage
  const batteryPlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  const readerPidPlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  currentTop += DEFAULT_HEIGHT / 2;
