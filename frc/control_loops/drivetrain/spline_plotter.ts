...
export function plotSpline(conn: Connection, element: Element): void {
  ...
  const longitudinalPlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  ...
  longitudinalPlot.addMessageLine(status, [
    'trajectory_logging',
    'distance_remaining',
  ]);

  const boolPlot = aosPlotter.addPlot(element, [
    DEFAULT_WIDTH,
    DEFAULT_HEIGHT / 2,
  ]);
  currentTop += DEFAULT_HEIGHT / 2;
  ...
  boolPlot
    .addMessageLine(status, ['trajectory_logging', 'is_executing'])
    .setColor(RED);
  boolPlot
    .addMessageLine(status, ['trajectory_logging', 'is_executed'])
    .setColor(BLUE);
