...
def main(argv):
    ...
    plant = linear_system.LinearSystem(kElevator)

    # volts / meter -> volts / rotation
    # 1 rotation * kElevator.G * radius * 2.0 * pi -> meters
    ...
    meters_per_rotation = (kElevator.G * kElevator.radius * 2.0 * numpy.pi)

    Ka = plant.B_continuous[1, 0]

    # meters/rotation -> 1.0 / (kElevator.G * kElevator.radius * 2.0 * math.pi)
    ...
    glog.info('Kg: %s volts',
              (kElevator.mass - 5.0 / 2.2) * 9.8 * kElevator.radius *
              kElevator.G / kElevator.motor.Kt * kElevator.motor.resistance)
