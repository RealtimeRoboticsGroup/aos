 ...
 public:
  AutoGoalSender(::aos::EventLoop *event_loop)
      : superstructure_goal_sender_(
        ...
        autonomous_pd_loop_(nullptr) {
    std::string_view path_to_trajectory = "trajectories/forward_slowly.json";

    std::vector<std::pair<double, std::function<void()>>> callbacks = {/*
        {0.5, [this]() { set_goals(PivotGoal::AMP, IntakeGoal::NONE); }},
        {1.5, [this]() { set_goals(PivotGoal::AMP, IntakeGoal::SCORE); }},
